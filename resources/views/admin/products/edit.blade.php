...
<label>Add more product images <small>(optional, maximum 10)</small><span class="admin-product-images"><img class="table-thumb" src="{{ Storage::url($product->image_path) }}" alt="">@foreach($product->images as $image)<img class="table-thumb" src="{{ Storage::url($image->image_path) }}" alt="">@endforeach</span><input type="file" name="images[]" accept="image/jpeg,image/png,image/webp" multiple>@error('images.*')<small class="field-error">{{ $message }}</small>@enderror</label>
...
<label><span>Cash on delivery</span><span class="admin-check"><input type="hidden" name="cod_available" value="0"><input type="checkbox" name="cod_available" value="1" @checked(old('cod_available', $product->cod_available))> Available for this product</span></label>
