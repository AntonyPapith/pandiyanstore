...
<head>
    ...
    <title>Login | Pandiyan Store</title>
...
<body class="login-page">
    <div class="login-card"><a href="{{ route('home') }}"><img src="{{ asset('logo.png') }}" alt="Pandiyan Store"></a>
        ...
        <form method="POST" action="{{ route('login.store') }}">@csrf
            ...
            <label class="remember"><input type="checkbox" name="remember" value="1"> Remember me</label><button class="primary-btn" type="submit">Login</button>
        </form>
        <a class="forgot-password-link" href="{{ route('password.request') }}">Forgot password?</a>
        <p class="auth-switch">New customer? <a href="{{ route('customer.register') }}">Create account</a></p><a class="back-home" href="{{ route('home') }}">&larr; Back to store</a>
    </div>
