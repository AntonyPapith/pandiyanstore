...
class OrderController extends Controller
{
    ...
    {
        $data = $request->validate(['payment_method' => ['required', Rule::in(['cod', 'upi'])]]);
        $cart = $request->session()->get('cart', []);
        if ($data['payment_method'] === 'upi') {
            return redirect()->route('payment')->withErrors(['payment' => 'Please use the UPI checkout button again.']);
        }
        $order = $orders->place($request->user(), $cart, 'cod');
